...
$counts = $table->fetch_user_log_count_desc($start, $perpage);
$records = $table->fetch_all_by_uid(array_keys($counts));
foreach ($counts as $key => $item) {
    ...
    foreach ($records[$key] as $row) {
        showtablerow('', [
            ...
            'title="' . dhtmlspecialchars($item['uid']) . '"',
            'title="' . dhtmlspecialchars($item['username']) . '"',
